...
Note that CartQuill does **not** use Freemius's own `__premium_only` stripping — this repo splits the editions itself and uploads only the premium package. Freemius must therefore never be allowed to deploy a free version anywhere: it has no markers to act on, so whatever it generates is not a core build. WordPress.org gets `build/cartquill` from `release.yml`, and nothing else.
...
**Freemius decides what a store has paid for; nothing local does.** `FreemiusBridge` *overrides* rather than unions on a premium build, and `cartquill_fs_owns_plan()` is true wherever `src/freemius.php` exists — deliberately independent of whether the SDK instance came up, so the answer does not change with what happens to be on disk. `LicensePage` renders read-only in that mode. The `CARTQUILL_LOCAL_LICENSE` constant in `wp-config.php` is the only way back to local keys, and it is there so the add-ons can be built and demoed. Do not soften any of this: `OptionLicense` is a development store that verifies nothing, and every guard above exists to keep it out of the decision.
...
The same release also uploads the premium package to Freemius, which is where paying customers' updates come from. It needs exactly one repository secret, `FREEMIUS_API_KEY` — the product-scoped bearer token from *dashboard.freemius.com → the product's Settings → API & Keys → "API Bearer Authorization Token"*. The v1 API needs nothing else for a deployment: no developer id, and not the secret key. Without it the step warns and skips rather than failing, since WordPress.org has already been published by that point and a red job would misreport a release that shipped.

It is a plain `curl` to `POST /v1/products/{id}/tags.json` (multipart, field `file`; omit `data` for WordPress plugins) rather than the community GitHub Action, for two reasons: the action would hold the credential, and it cannot report a failure — both of its error paths call bare `die()` and its `catch` only prints, so all three exit 0 and a rejected upload would land as a green release with nothing deployed. The step checks the HTTP status and that the version Freemius recorded matches the one being released.

Deployments default to `release_mode: pending`, which is why there is no follow-up `PUT`. Promoting a version is a deliberate click in the dashboard, because a paid update reaches live stores with no directory review in front of it; staged and incremental rollouts live on that same `PUT /v1/products/{id}/tags/{tag_id}.json` and are the right way to ship to real stores. **Freemius credentials belong in GitHub secrets and nowhere else** — never in plugin code, never in a build.
