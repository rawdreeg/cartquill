...
The same release also uploads the premium package to Freemius, which is where paying customers' updates come from. It needs the `FREEMIUS_DEV_ID`, `FREEMIUS_PLUGIN_ID`, `FREEMIUS_PUBLIC_KEY` and `FREEMIUS_SECRET_KEY` repository secrets; without them the step warns and skips rather than failing, since WordPress.org has already been published by that point and a red job would misreport a release that shipped. **The secret key belongs in GitHub secrets and nowhere else** — never in plugin code, never in a build. Versions land as `pending`: promoting one to Released is a deliberate click in the Freemius dashboard, because a paid update reaches live stores with no directory review in front of it. Staged rollouts live there too and are the right way to ship to real stores.
