...
Deployments default to `release_mode: pending`, which is why there is no follow-up `PUT`. Promoting a version is a deliberate click, because a paid update reaches live stores with no directory review in front of it; staged and incremental rollouts live on that same `PUT /v1/products/{id}/tags/{tag_id}.json`. **The secret key belongs in GitHub secrets and nowhere else** — never in plugin code, never in a build. Versions land as `pending`: promoting one to Released is a deliberate click in the Freemius dashboard, because a paid update reaches live stores with no directory review in front of it. Staged rollouts live there too and are the right way to ship to real stores.
